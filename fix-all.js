const fs = require('fs');
const pageFile = 'src/app/(teaching-record)/TeachingRecordPage/page.jsx';
const formFile = 'src/app/(components)/TeachingRecordForm.jsx';
...
`;
fs.writeFileSync(pageFile, fixedPageContent);

// 2. Fix TeachingRecordForm.jsx using Regex to accept HEAD for every conflict
let formContent = fs.readFileSync(formFile, 'utf8');
// The pattern matches <<<<<<< HEAD\n, then non-greedy capture, then =======, then non-greedy skip until >>>>>>> ...
const conflictRegex = /<<<<<<< HEAD\n([\s\S]*?)=======\n[\s\S]*?>>>>>>> [^\n]*\n/g;
const fixedFormContent = formContent.replace(conflictRegex, '$1');
fs.writeFileSync(formFile, fixedFormContent);
