...
import { Activity, Users, Stethoscope, Baby, Building2, Brain, Flame, HeartCrack, Frown, Shield, HeartHandshake } from 'lucide-react';
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-sky-200">
      ...
      <div className="relative z-10 max-w-[1600px] w-full mx-auto space-y-12 py-12 px-4 sm:px-6 lg:px-8">
        {/* Header Section */}
        <div className="text-center space-y-6 bg-white/40 backdrop-blur-3xl p-8 rounded-[2rem] shadow-[0_8px_32px_rgba(0,0,0,0.1)] border border-white/60 relative overflow-hidden">
          {/* Subtle gradient glow inside header */}
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-1/2 bg-gradient-to-b from-white/40 to-transparent pointer-events-none rounded-t-[2rem]"></div>
          ...
          <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight flex flex-col md:flex-row items-center justify-center gap-3 relative z-10">
            ...
            <StaggeredText text="MENTAL HEALTH CHECK IN" className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-pink-500" staggerDuration={0.03} delay={0.5} />
          </h1>
            ...
            {assessmentLinks.map((item, index) => (
              <Card key={index} className="hover:-translate-y-2 transition-all duration-400 border border-white/60 bg-white/40 backdrop-blur-2xl shadow-[0_8px_30px_rgba(0,0,0,0.08)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.12)] hover:bg-white/50 rounded-3xl overflow-hidden group">
                <CardBody className="p-8 flex flex-col items-center text-center space-y-5">
                  <div className="p-5 bg-gradient-to-br from-white to-blue-50 rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.05)] border border-white group-hover:scale-110 transition-transform duration-300">
                    {item.icon}
                  </div>
                  ...
                  <Button
                    as={Link}
                    href={item.url}
                    className="w-full mt-4 font-bold bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg hover:shadow-indigo-500/40 rounded-xl py-6 text-md"
                  >
                    เริ่มทำแบบประเมิน
                  </Button>
                </CardBody>
