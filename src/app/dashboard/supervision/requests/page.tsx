...
export default function SupervisionRequestsPage() {
  ...
  const fetchRecords = async (queryTermYear = "") => {
    setIsLoading(true);
    try {
      let url = "/api/supervision?status=รอตรวจสอบ";
      if (queryTermYear) {
  ...
  if (status === "loading" || (status === "authenticated" && permissions === null)) return null;
  const isSuperAdmin = session?.user?.role === "super_admin";
  const hasPermission = permissions?.manage_supervision_requests || isSuperAdmin;
    ...
    return (
      <div className="min-h-screen bg-zinc-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-3xl shadow-xl text-center space-y-4 max-w-sm w-full">
          ...
          <h2 className="text-xl font-black text-zinc-900">ปฏิเสธการเข้าถึง</h2>
          <p className="text-sm font-bold text-zinc-500">
            คุณไม่ได้รับสิทธิ์ให้เข้าถึงหน้าจัดการคำร้องนิเทศ
          </p>
          <button
            onClick={() => router.back()}
            className="w-full py-3 bg-zinc-900 text-white rounded-xl font-black text-sm hover:bg-zinc-800 transition-colors"
          >
            ย้อนกลับ
          </button>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-zinc-900 p-6 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-800">
          <div>
            <h1 className="text-2xl font-black text-zinc-900 dark:text-white">
              คำร้องรอการอนุมัติ: ระบบนิเทศการสอน
            </h1>
            <p className="text-sm text-zinc-500 font-bold mt-1">
              ตรวจสอบและจัดการรายการนิเทศที่รอการอนุมัติ
            </p>
          </div>
