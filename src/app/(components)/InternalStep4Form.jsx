...
const InternalStep4Form = ({ projectId, initialData = {} }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    subject: initialData.subject || "ขออนุมัติคำสั่งแต่งตั้งคณะกรรมการดำเนินงาน",
    salutation: initialData.salutation || "ผู้อำนวยการวิทยาลัยเทคนิคกันทรลักษ์",
    paragraphs:
      initialData.paragraphs && initialData.paragraphs.length > 0 ? initialData.paragraphs : [""],
    introPrefix: initialData.introPrefix || "",
    ...
    deputy2Name: initialData.deputy2Name || "",
    deputy2Position: initialData.deputy2Position || "",
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/InternalPdcas/${projectId}/step4`, {
        method: "POST",
  ...
  const handleExportPDF = () => {
    ...
    const clean = (text) => {
      if (!text) return "";
      return toThaiDigits(text.trim().replace(/^[ \u0e48-\u0e4b|'‘’"“”]+/g, ""));
    };
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>บันทึกขออนุมัติคำสั่ง - ${formData.projectName}</title>
          <style>
            @font-face {
        ...
        <body>
          ...
          <div class="para">
            ...
            ${toThaiDigits((formData.additionalIntroText || "").trim())} จึงขออนุมัติคำสั่งแต่งตั้งคณะกรรมการดำเนินงาน โครงการ ${clean(formData.projectName)} ดังกล่าว รายละเอียดตามร่างคำสั่งที่แนบมาพร้อมนี้
          </div>
          
          <div style="margin-top: 15px; text-indent: 2.5cm;">จึงเรียนมาเพื่อโปรดพิจารณาอนุมัติ</div>
    ...
    <div className="rounded-3xl border border-stroke bg-white p-8 shadow-xl dark:bg-boxdark">
      <div className="mb-8 flex items-center justify-between border-b pb-6">
        <h2 className="text-2xl font-black text-primary">
          4. บันทึกขออนุมัติคำสั่งแต่งตั้งคณะกรรมการดำเนินงาน
        </h2>
      ...
      <div className="space-y-10">
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div className="space-y-2">
              <label className="block text-sm font-bold text-gray-600">วันที่</label>
              <input
                name="date"
                value={formData.date}
            ...
            <div className="space-y-2 md:col-span-2">
              <label className="block text-sm font-bold text-gray-600">เรื่อง</label>
              <input
                name="subject"
            ...
            <div className="space-y-2 md:col-span-2">
              <label className="block text-sm font-bold text-gray-400">เรียน - [ล็อค]</label>
              <input
                name="salutation"
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4 rounded-2xl bg-blue-50 p-6 shadow-inner">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-blue-600">คำนำหน้า</label>
                  <input
                    name="introPrefix"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-blue-600">ชื่อแผนก/งาน</label>
                  <input
                    name="departmentName"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-blue-600">ชื่อโครงการ</label>
                  <input
                    name="projectName"
              ...
              <div className="space-y-2 border-t border-blue-100 pt-4">
                <label className="block text-sm font-black text-blue-800">
                  เขียนข้อความวัตถุประสงค์เพิ่มเติม (จะแสดงก่อนคำว่า
                  'จึงขออนุมัติคำสั่งแต่งตั้งคณะกรรมการ...')
                </label>
                <textarea
                  ...
                  placeholder="เช่น เพื่อประโยชน์และให้การดำเนินงานโครงการดังกล่าวเป็นไปด้วยความเรียบร้อย มีประสิทธิภาพ จึงได้จัดทำร่างคำสั่งแต่งตั้งคณะกรรมการดำเนินงาน"
                />
              </div>
            </div>
            ...
            <div className="mt-4 rounded-2xl border-2 border-primary/20 bg-primary/5 p-6 shadow-inner">
              <h4 className="mb-3 flex items-center gap-2 text-sm font-black text-primary">
                ตัวอย่างย่อหน้าสรุปที่จะแสดงใน PDF:
              </h4>
              <div className="space-y-4 text-lg leading-relaxed text-gray-700">
                <div>
                  {formData.introPrefix || "ในการนี้"} {formData.departmentName || "ชื่อแผนก/ฝ่าย"}{" "}
                  {formData.additionalIntroText || ""} จึงขออนุมัติคำสั่งแต่งตั้งคณะกรรมการดำเนินงาน
                  โครงการ {formData.projectName || "ชื่อโครงการ"} ดังกล่าว
                  รายละเอียดตามร่างคำสั่งที่แนบมาพร้อมนี้
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-8">
            <div className="space-y-4 rounded-3xl border-2 border-dashed border-gray-200 p-6">
              <h4 className="font-black text-gray-500">ส่วนของ: รองผู้อำนวยการ</h4>
        ...
        <div className="mt-10 border-t pt-10 text-center">
          ...
          >
            {loading ? "กำลังบันทึก..." : "บันทึกข้อมูล (Step 4)"}
          </button>
        </div>
      </div>
...
export default InternalStep4Form;
