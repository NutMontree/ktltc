...
export default function PaNew() {
  ...
  const [logs, setLogs] = useState<IPaLog[]>([]);
  ...
  const fetchLogs = async () => {
    try {
      const res = await fetch("/api/pa/logs");
      if (res.ok) {
  ...
  const handleEdit = (doc: IPaDocument) => {
    setIsEditMode(true);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/pa/${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEditMode ? `/api/pa/${currentId}` : "/api/pa";
  ...
  // --- Log Handlers ---
  const handleEditLog = (log: IPaLog) => {
  ...
  const handleLogSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/pa/logs/${editLogData.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteLog = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/pa/logs/${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  return (
    <div className="max-w-[1600px] mx-auto py-12 w-full">
      <motion.div
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true }}
      ...
      >
        ...
        {isLoading ? (
          ...
          <div className="text-center py-10 text-slate-500">ไม่พบเอกสารระบบ PA</div>
        ) : (
          <Accordion
            variant="splitted"
              ...
              <form onSubmit={handleSubmit}>
                <div className="p-6 border-b border-slate-100 dark:border-zinc-800 flex justify-between items-center bg-slate-50/50 dark:bg-zinc-950/50">
                  <h3 className="text-xl font-bold text-slate-800 dark:text-white">
                    {isEditMode ? "แก้ไขเอกสาร PA" : "เพิ่มเอกสาร PA"}
                  </h3>
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="เช่น รายงานการปฏิบัติงาน (PA) ปี 2566 ของ นาย ก."
