...
// Escape regex special chars, then collapse whitespace into \s+ for flexible matching
function toFlexRegex(input: string): string {
  ...
  try {
    ...
    const students = await db
      ...
      .find({
        ...
        $or: [
          ...namePatterns,
          { studentIdNum: { $regex: flexQ, $options: "i" } },
          { classroomName: { $regex: flexQ, $options: "i" } },
          { classGroupId: { $regex: flexQ, $options: "i" } },
          { department: { $regex: flexQ, $options: "i" } },
        ],
      })
      .limit(20)
      .project({
