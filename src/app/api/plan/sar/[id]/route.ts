...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await db.collection("sar_documents").updateOne(
    ...
    console.error("PUT SAR Document Error:", error);
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await db.collection("sar_documents").deleteOne({
      _id: new ObjectId((await params).id),
    });
    ...
    console.error("DELETE SAR Document Error:", error);
