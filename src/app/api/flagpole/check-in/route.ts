...
export async function POST(req: Request) {
  try {
    ...
    if (!userId) {
      return NextResponse.json({ success: false, message: 'กรุณาเข้าสู่ระบบก่อนดำเนินการ' }, { status: 401 });
    }
    ...
    // 2. ดึงการตั้งค่าเพื่อตรวจสอบพิกัด
    const globalSetting = await db.collection("role_settings").findOne({ role: "system_global" });
    const inSiteThreshold = globalSetting?.inSiteDistance || DEFAULT_IN_SITE_DISTANCE;
    ...
    // ขอบเขตเวลาสำหรับการเข้าแถวหน้าเสาธง
    const flagStart = 700;   // 07:00 น.
    const flagLate = 800;    // 08:00 น.
    ...
    if (currentTimeVal < flagStart) {
      return NextResponse.json({ 
        success: false, 
        message: 'ยังไม่ถึงเวลาลงชื่อเข้าแถว (ระบบเปิดให้บริการเวลา 07:00 น.)' 
      }, { status: 403 });
    }
    ...
    if (currentTimeVal > flagClose) {
      return NextResponse.json({ 
        success: false, 
        message: 'หมดเวลาลงชื่อเข้าแถวหน้าเสาธงแล้ว (ปิดระบบเวลา 08:45 น.)' 
      }, { status: 403 });
    }
    ...
    return NextResponse.json({ success: false, message: error.message || 'เกิดข้อผิดพลาดในการลงชื่อเข้าแถว' }, { status: 500 });
  }
}
