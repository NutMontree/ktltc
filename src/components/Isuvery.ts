...
export interface Isuvery {
    _id: string; // ID สำหรับ MongoDB
    createdAt?: Date | string; // วันที่สร้าง (Mongoose timestamps)
    updatedAt?: Date | string; // วันที่อัปเดต (Mongoose timestamps)
    submittedAt?: Date | string; // วันที่กรอก (กำหนดเอง)
