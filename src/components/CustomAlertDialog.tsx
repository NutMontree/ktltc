...
const CustomAlertDialog: FC<CustomAlertDialogProps> = ({
        ...
        switch (type) {
            ...
            default:
                return {
                    ...
                    bg: 'bg-indigo-50',
                    icon: (
                        <svg className="w-6 h-6 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    )
                };
        }
    };
    ...
    return (
        ...
        >
            {/* Modal Content */}
            <div className={`w-full max-w-sm rounded-xl p-6 shadow-2xl transition-transform duration-300 transform scale-100 bg-white border ${styles.bg}`}>
                ...
                <div className="flex justify-end space-x-3">
                    ...
                    <button
                        ...
                        className={`px-4 py-2 font-semibold rounded-lg transition shadow-md 
                                   ${styles.color.replace('text-', 'bg-').replace('-700', '-600')} 
                                   ${styles.color.replace('text-', 'hover:bg-').replace('-700', '-700')} 
                                   text-white`}
                    >
                        {confirmText}
                    </button>
                    ...
                    {onConfirm && (
                        <button
                            onClick={onClose}
                            className="px-4 py-2 text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 transition shadow-md"
                        >
                            ยกเลิก
                        </button>
                    )}
